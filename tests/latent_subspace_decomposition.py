...
severities = [float(v) for v in (sys.argv[4] if len(sys.argv) > 4 else "0.5").split(",")]
N_EVENTS = 20000
...
z_c = embed()
# probe is fit ONCE on clean latents and reused for every severity, exactly as the
# grader does: it never sees a degraded latent.
