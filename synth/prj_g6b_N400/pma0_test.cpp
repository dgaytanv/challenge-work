...
int main(int argc, char **argv) {
    ...
    if (fin.is_open() && fpr.is_open()) {
      ...
      x_feat_t x_feat[400*14];
      nnet::copy_data<float, x_feat_t, 0, 400*14>(in, x_feat);
      mask_add_t mask_add[400*32];
      nnet::copy_data<float, mask_add_t, 5600, 400*32>(in, mask_add);
      result_t layer37_out[6];
        ...
        for (unsigned i = 0; i < NUM_TEST_SAMPLES; i++) {
            ...
            x_feat_t x_feat[400*14];
            nnet::fill_zero<x_feat_t, 400*14>(x_feat);
            mask_add_t mask_add[400*32];
            nnet::fill_zero<mask_add_t, 400*32>(mask_add);
            result_t layer37_out[6];
