#!/bin/bash
# Evaluate a-pma-transformer two ways, from a fresh clone of the code that builds it.
#
...
echo "== cloning submission-a-pma @ d450bcf"
git clone -q --branch submission-a-pma "$REPO" "$WORK/repo"
cd "$WORK/repo"
git checkout -q d450bcf

CKPT="$WORK/repo/checkpoints/a_pma_submission.pth"   # ships on the branch; over the 5 MB cap here
sha256sum -c "$HERE/None.sha256" 2>/dev/null || true
